...
if do_indexing:  # approx ~10Gb each indexing
    ...
    c.execute("""CREATE INDEX gaia_source_sourceid ON gaia_source (source_id);""")
    # 47m15s
    ...
    # 117m17s
    print("Start doing gaia_source_phot_g_mean_flux indexing")
    c.execute(
        """CREATE INDEX gaia_source_phot_g_mean_flux ON gaia_source (phot_g_mean_flux);"""
    )
    # 58m4s
    ...
    # 44m22s
    c.execute(
        """CREATE INDEX gaia_source_rv_expected_sig_to_noise ON gaia_source (rv_expected_sig_to_noise);"""
    )
    # 34m19s
    c.execute(
        """CREATE INDEX gaia_source_has_xp_continuous ON gaia_source (has_xp_continuous);"""
    )
    # 33m32s
    c.execute(
