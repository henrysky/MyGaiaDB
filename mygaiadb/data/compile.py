...
import numpy as np
import pandas as pd
from . import _GAIA_DR3_GAIASOURCE_PARENT, _GAIA_DR3_ALLWISE_NEIGHBOUR_PARENT, _GAIA_DR3_2MASS_NEIGHBOUR_PARENT
from .. import astro_data_path, gaia_sql_db_path, gaia_xp_coeff_h5_path


def compile_xp_continuous_allinone_h5():
    base_path = astro_data_path.joinpath("gaia_mirror", "Gaia", "gdr3", "Spectroscopy", "xp_continuous_mean_spectrum",)
...
def compile_xp_continuous_h5(save_correlation_matrix=False):
    for i_path in tqdm.tqdm(astro_data_path.joinpath("Gaia/gdr3/Spectroscopy/xp_continuous_mean_spectrum/").glob("*.csv.gz"), desc="XP coeffs"):
        file_path_f = ascii.read(i_path)

        bp_basis_function_id = np.asarray(file_path_f["bp_basis_function_id"], dtype=np.int16)
        bp_degrees_of_freedom = np.asarray(file_path_f["bp_degrees_of_freedom"], dtype=np.int16)
        ...
        bp_n_rejected_measurements = np.asarray(file_path_f["bp_n_rejected_measurements"], dtype=np.int16)
        bp_standard_deviation = np.asarray(file_path_f["bp_standard_deviation"], dtype=np.float32)
        bp_chi_squared = np.asarray(file_path_f["bp_chi_squared"], dtype=np.float32)
        if save_correlation_matrix:
            bp_coefficient_correlations = np.stack(file_path_f["bp_coefficient_correlations"])
        bp_coefficient_errors = np.stack(file_path_f["bp_coefficient_errors"])
        bp_coefficients = np.stack(file_path_f["bp_coefficients"])
        bp_n_relevant_bases = np.asarray(file_path_f["bp_n_relevant_bases"], dtype=np.int16)
        bp_relative_shrinking = np.asarray(file_path_f["bp_relative_shrinking"], dtype=np.float32)

        rp_basis_function_id = np.asarray(file_path_f["rp_basis_function_id"], dtype=np.int16)
        rp_degrees_of_freedom = np.asarray(file_path_f["rp_degrees_of_freedom"], dtype=np.int16)
        ...
        rp_n_rejected_measurements = np.asarray(file_path_f["rp_n_rejected_measurements"], dtype=np.int16)
        rp_standard_deviation = np.asarray(file_path_f["rp_standard_deviation"], dtype=np.float32)
        rp_chi_squared = np.asarray(file_path_f["rp_chi_squared"], dtype=np.float32)
        if save_correlation_matrix:
            rp_coefficient_correlations = np.stack(file_path_f["rp_coefficient_correlations"])
        rp_coefficient_errors = np.stack(file_path_f["rp_coefficient_errors"])
        rp_coefficients = np.stack(file_path_f["rp_coefficients"])
        rp_n_relevant_bases = np.asarray(file_path_f["rp_n_relevant_bases"], dtype=np.int16)
        rp_relative_shrinking = np.asarray(file_path_f["rp_relative_shrinking"], dtype=np.float32)
        ...
        with h5py.File(f"./xp_continuous_mean_spectrum/{file_names_wo_ext}.h5","w") as h5f:
            h5f.create_dataset("source_id",
                                data=file_path_f["source_id"].data)
            h5f.create_dataset("solution_id",
                                data=file_path_f["solution_id"].data)
...
def compile_rvs_h5():
    for i_path in tqdm.tqdm(astro_data_path.joinpath("Gaia/gdr3/Spectroscopy/rvs_mean_spectrum/").glob("*.csv.gz"), desc="RVS spec"):
        file_path_f = ascii.read(i_path)
        ...
        with h5py.File(f"./rvs_mean_spectrum/{file_names_wo_ext}.h5","w") as f:
            ...
            f.create_dataset("solution_id",
                            data=file_path_f["solution_id"].data)
            f.create_dataset("ra",
                            data=file_path_f["ra"].data)
            f.create_dataset("dec",
                            data=file_path_f["dec"].data)
...
def comile_xp_mean_spec_h5():
    for i_path in tqdm.tqdm(astro_data_path.joinpath("Gaia/gdr3/Spectroscopy/xp_sampled_mean_spectrum/").glob("*.csv.gz"), desc="XP specs"):
        file_path_f = ascii.read(i_path)
        flux = np.vstack(file_path_f["flux"])
        flux_error = np.vstack(file_path_f["flux_error"])
...
def compile_gaia_sql_db(do_gaia_source_table=True, do_indexing=True):
    ...
    if do_gaia_source_table:
        ...
        for name in [_GAIA_DR3_ALLWISE_NEIGHBOUR_PARENT, _GAIA_DR3_2MASS_NEIGHBOUR_PARENT]:
